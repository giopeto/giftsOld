...
giftsControllers.controller('MakeGiftController', ['$scope', '$http', '$location',
  function($scope, $http, $location) {
	//$scope.gift = {};
	
	$scope.saveGiftInfo = function() {
	
		$scope.gift.disabled = false;
		//$scope.gift = {};
		...
		$http({
			...
			$scope.gift.error.userIdError = data.userIdError;
			$scope.gift.error.cityError = data.cityError;
			$scope.gift.error.nameError = data.nameError;
			
			if (data.successGiftInfo) {
				$scope.gift = {};
				$location.path("/#viewGifts/");
			}
...
giftsControllers.controller('uploadFileController', ['$scope', '$http',
	...
	$scope.uploadFile = function(){
		...
		if (!$scope.gift.name) {
			$scope.gift.error.nameError = "Името е задължително поле";
			return;
		}
		$scope.gift.disabled = true;
